...
(async () => {
  for (const notes of samples) {
    ...
    if (!Array.isArray(notes)) {
      console.error(`Run ${runs}: notes is not array, type=${typeof notes}`);
      continue;
    }
    if (notes.length === 0) {
      console.error(`Run ${runs}: notes array is empty`);
      continue;
    }
    for (let i = 0; i < notes.length; i++) {
      const n = notes[i];
      if (!n || typeof n !== 'object' || Array.isArray(n)) {
        console.error(`Run ${runs}: note ${i} invalid:`, typeof n, JSON.stringify(n));
        break;
      }
      if (typeof n.id !== 'string' || n.id.length === 0) {
        console.error(`Run ${runs}: note ${i} invalid id:`, n.id);
        break;
      }
      if (typeof n.content !== 'string') {
        console.error(`Run ${runs}: note ${i} invalid content:`, typeof n.content);
        break;
      }
      if (!Array.isArray(n.tags)) {
        console.error(`Run ${runs}: note ${i} invalid tags:`, typeof n.tags);
        break;
      }
    }
    ...
    try {
      ...
      if (index.notes.length !== notes.length) {
        console.error(`Run ${runs}: index length mismatch: ${index.notes.length} vs ${notes.length}`);
      }
    } catch (err) {
      console.error(`Run ${runs}: buildIndex error:`, err.message);
    }
  }
